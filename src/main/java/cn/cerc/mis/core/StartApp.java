...
public class StartApp implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (staticFile.isStaticFile(uri)) {
            String url = "/forms" + uri.substring(uri.indexOf("/", 2));
        ...
        String token = (String) req.getAttribute(RequestData.appSession_Key);

        String childCode = getRequestCode(reqPath, token);
        if (childCode == null) {
    ...
    protected String getRequestCode(String reqPath, String token) {
        String url = null;
        String args[] = reqPath.split("/");
